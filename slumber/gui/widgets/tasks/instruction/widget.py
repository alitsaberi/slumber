...
class EmptyWebPage(TaskPage, Ui_EmptyWebPage):
    def __init__(self, index: int, title: str, instruction_path: Path | str, parent: QWidget = None):
        ...
        if not self.instruction_path.exists():
            raise FileNotFoundError(f"Instruction file not found: {self.instruction_path}")
